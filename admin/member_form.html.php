...
// 会員登録と会員編集の共通HTMLテンプレート

// member_form.phpから以下の変数が渡される:
// $title, $formData, $errors, $stage, $is_edit_mode, $member_id, 
// $form_action_url, $action_button_value, $action_button_label, $token, PREFECTURES, GENDERS
...
<body>
    <div class="container">
        ...
        ?>
            ...
            <!-- フォームの送信先を動的に設定 -->
            <form action="<?= htmlspecialchars($form_action_url) ?>" method="POST">
                ...
                <div class="form-group name-group">
                    <div class="id-field">
                        ...
                        <!-- 編集モードではIDを表示、登録モードでは「自動採番」と表示 -->
                        <?php if ($is_edit_mode): ?>
                            <span class="id-value"><?= (int)$member_id ?></span>
                        <?php else: ?>
                            <span class="id-auto-number">登録後に自動採番</span>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="radio-group">
                        <label>
                            <input type="radio" name="gender" value="1" <?= ((int)($formData['gender'] ?? 0) === 1) ? 'checked' : '' ?>>
                            <span>男性</span>
                        </label>
                        <label>
                            <input type="radio" name="gender" value="2" <?= ((int)($formData['gender'] ?? 0) === 2) ? 'checked' : '' ?>>
                            <span>女性</span>
                        </label>
                    </div>
                ...
                <div class="form-group">
                    <div class="pass-field">
                        ...
                        <input type="password" id="password" name="password" value="<?= htmlspecialchars($formData['password'] ?? '') ?>" class="form-input <?= isset($errors['password']) ? 'is-error' : '' ?>">
                        <!-- <?php if ($is_edit_mode): ?>
                            <p class="guide-text">※ 変更しない場合は空欄にしてください</p>
                        <?php endif; ?> -->
                    ...
                    <div class="pass-field">
                        <label for="password_confirm" class="form-label">パスワード確認</label>
                         <input type="password" id="password_confirm" name="password_confirm" value="<?= htmlspecialchars($formData['password_confirm'] ?? '') ?>" class="form-input <?= isset($errors['password_confirm']) ? 'is-error' : '' ?>" placeholder="確認のため再入力">
        ...
        ?>
            ...
            $displayFields = [
                // IDは編集モードでのみ表示
                'ID' => $is_edit_mode ? (int)$member_id : '登録後に自動採番',
                '氏名' => htmlspecialchars($formData['name_sei']) . ' ' . htmlspecialchars($formData['name_mei']),
                '性別' => htmlspecialchars(GENDERS[(int)($formData['gender'] ?? 0)] ?? '未選択'),
                '住所' => htmlspecialchars($formData['pref_name'] ?? '') . ' ' . htmlspecialchars($formData['address'] ?? ''),
                // パスワード: 編集モードで入力がなければ「セキュリティのため非表示」
                // 登録モードまたは編集モードで入力があれば「セキュリティのため非表示」
                'パスワード' => ($is_edit_mode && empty($formData['password'])) 
                    ? '<span class="password-hidden">セキュリティのため非表示</span>' 
                    : '<span class="password-hidden">セキュリティのため非表示</span>',
            ...
            <div class="button-container button-group">
                <!-- 戻るボタンの送信先を動的に設定 -->
                <form action="<?= htmlspecialchars($form_action_url) ?>" method="POST" style="flex: 1; margin-right: 10px;">
                ...
                <div class="verification_screen_button" style="flex: 1;">
                    <!-- 完了ボタンの送信先とactionを動的に設定 -->
                    <form action="<?= htmlspecialchars($form_action_url) ?>" method="POST">
                        <input type="hidden" name="action" value="<?= $action_button_value ?>">
                        <input type="hidden" name="token" value="<?= htmlspecialchars($token) ?>">
